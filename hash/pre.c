...
int main(int argc, char**argv){
	...
	char *fname = (char*)malloc(sizeof(char)*(strlen(PATH)+strlen(argv[1])+strlen(argv[2])+1));
	strcpy(fname,PATH);
	strcat(fname,argv[1]);
	strcat(fname,"_");
	strcat(fname,argv[2]);
	...
	fclose(file);
	fname = (char*)realloc(fname,sizeof(char)*(strlen(PATH)+strlen(argv[1])+strlen(temp)+2));
	strcpy(fname,PATH);
    strcat(fname,argv[1]);
    strcat(fname,"_");
    strcat(fname,temp);
